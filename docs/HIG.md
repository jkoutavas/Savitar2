...
| Item | Shortcut | Behavior |
|------|----------|----------|
| **Savitar Help** | ⌘? | Opens the bundled user guide in a Savitar window (`HelpGuideWindowController` + `WKWebView`). Works offline. |
| **Savitar Guide on the Web…** | — | Opens `docs/USER_GUIDE.md` on GitHub (interim until heynow.com guide ships). |
| **About Privacy…** | — | Opens Savitar Help at `#privacy` (TelemetryDeck disclosure). |
| **Release Notes…** | — | Opens GitHub Releases. |

**Do not** route primary help through macOS Help Viewer (`NSHelpManager` / `helpd`) — it is unreliable for Xcode and non–`/Applications` builds. The `.help` bundle remains in the app for indexing and a possible future `registerBooks` integration.

## Contextual help (? buttons)

Major windows expose a small **?** control that jumps to the matching guide anchor (Story 17):

| Surface | Placement | API |
|---------|-----------|-----|
| World Picker, world session, Events, App Settings | Title bar trailing | `SavitarHelpButton.installInTitleBar(of:for:)` |
| World Settings sheet | Top-trailing of sheet content | `SavitarHelpButton.installInTopTrailingCorner(of:for:)` |

App Settings updates the anchor when the toolbar pane changes. World Settings updates on tab change (KVO on `selectedTabViewItemIndex`).

Anchor IDs live in `SavitarHelp.Anchor` and `SavitarHelp.ContextualSurface`; HTML `id` values are generated by `client/scripts/build_help_book.py` from `docs/USER_GUIDE.md`.

### Code touchpoints

- `client/Savitar2/src/SavitarHelp.swift`
- `client/Savitar2/src/SavitarHelpButton.swift`
- `client/Savitar2/src/HelpGuideWindowController.swift`
- `client/scripts/build_help_book.py`
